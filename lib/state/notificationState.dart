...
class NotificationState extends AppState {
  ...
  String notificationReciverId, notificationPostId;
  List<FeedModel> notificationPostList;
  ...
  /// get `Post` present in notification
  Future<FeedModel> getPostDetail(String postId) async {
    FeedModel _postDetail;
    var snapshot =
        await kfirestore.collection(TWEET_COLLECTION).doc(postId).get();
    ...
    if (_postDetail == null) {
      cprint("Post not found " + postId);

      /// remove notification from firebase db if post in not available or deleted.
    }
    if (postId == "AOrRB0EHIbFSAev2WX4P") {
      print("dsfsfgg");
    }
    return _postDetail;
  }

  /// get user who liked your post
  ...
  void removeNotification(String userId, String postkey) async {
    print("removeNotification " + postkey);
    _userCollection
        .doc(userId)
        .collection(NOTIFICATION_COLLECTION)
        .doc(postkey)
        .delete();
    // kDatabase.child('notification').child(userId).child(postkey).remove();
  }

  /// Trigger when somneone like your post
  void _onNotificationAdded(DocumentSnapshot event) {
    if (event.data() != null) {
      var model = NotificationModel.fromJson(event.data());
      model.postKey = event.id;
      // event.data()["updatedAt"], event.data()["type"]);
      if (_notificationList == null) {
        _notificationList = List<NotificationModel>();
      }
      if (_notificationList.any((x) => x.postKey == model.postKey)) {
        return;
      }
  ...
  void _onNotificationChanged(DocumentSnapshot event) {
    if (event.data() != null) {
      ...
      _notificationList
          .firstWhere((x) => x.postKey == model.postKey)
  ...
  /// Trigger when someone undo his like on post
  void _onNotificationRemoved(DocumentSnapshot event) {
    if (event.data() != null) {
      var model = NotificationModel.fromJson(event.data());
      model.postKey = event.id;
      // remove notification from list
      _notificationList.removeWhere((x) => x.postKey == model.postKey);
